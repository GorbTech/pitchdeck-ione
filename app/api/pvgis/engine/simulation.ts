...
export function getBatteryDerating(
  nameplateKwh: number,
  tempC: number,
  conservativeOffset: number = 0  // negative offset for conservative mode (e.g. -5)
): BatteryDerating {
  ...
  // Conservative mode: assume heater underperforms by offset°C
  const effectiveTempC = Math.max(tempC + conservativeOffset, 5 + conservativeOffset);
...
export function simulateYear(
  ...
  grid: boolean,
  batteryTempOffset: number = 0  // conservative mode: -5°C offset
): SimulationResult {
  ...
  for (let pass = 0; pass < EC.MAX_WARMUP_PASSES; pass++) {
    ...
    for (let i = 0; i < climate.data.length; i++) {
      ...
      const derating = getBatteryDerating(batteryNominalKwh, p.temp_c, batteryTempOffset);
