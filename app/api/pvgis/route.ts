...
// Wind data result interface (single source)
...
// Merged wind result with conservative + median
interface MergedWindResult {
  conservative: WindDataResult;  // min(sources) per month — for main recommendation
  median: WindDataResult;        // median(sources) per month — for conditional scenario
...
interface OpenMeteoClimateExtras {
  monthlyMaxTemp: number[];   // 12 values: avg of daily Tmax per month (°C)
  monthlyMinTemp: number[];   // 12 values: avg of daily Tmin per month (°C)
  annualPrecipMm: number;     // total precipitation, annualized
  avgHumidityPct: number;     // annual avg relative humidity (%)
}
...
// NASA POWER: climate data fallback (temperature + precipitation)
// Used when Open-Meteo is unavailable (rate limited, etc.)
async function getNASAClimateData(lat: number, lon: number): Promise<OpenMeteoClimateExtras | null> {
  try {
    const year = new Date().getFullYear() - 2;  // Use last COMPLETE year (current year may be partial)
    const url = `https://power.larc.nasa.gov/api/temporal/monthly/point?parameters=T2M_MAX,T2M_MIN,PRECTOTCORR,RH2M&community=RE&longitude=${lon}&latitude=${lat}&start=${year}&end=${year}&format=JSON`;
    console.log('NASA POWER climate URL:', url);
    ...
    if (!response.ok) {
      console.log('NASA POWER climate failed:', response.status);
      return null;
    }
    ...
    if (!t2mMax || !t2mMin) {
      console.log('NASA POWER: no temperature data');
      return null;
    }

    // NASA monthly format: { "200501": value, "200502": value, ... "200513": annual }
    // Keys are YYYYMM, last key (13) is annual average — skip it
    const monthlyMaxTemp: number[] = [];
    const monthlyMinTemp: number[] = [];
    let totalPrecipMm = 0;
    let totalHumidity = 0;
    let humidityCount = 0;

    for (const [key, val] of Object.entries(t2mMax)) {
      const mm = parseInt(key.substring(4));
      if (mm >= 1 && mm <= 12) monthlyMaxTemp[mm - 1] = val as number;
    }
    for (const [key, val] of Object.entries(t2mMin)) {
      const mm = parseInt(key.substring(4));
      if (mm >= 1 && mm <= 12) monthlyMinTemp[mm - 1] = val as number;
    }
    if (precip) {
      const DAYS = [31,28,31,30,31,30,31,31,30,31,30,31];
      for (const [key, val] of Object.entries(precip)) {
        const mm = parseInt(key.substring(4));
        if (mm >= 1 && mm <= 12) totalPrecipMm += (val as number) * DAYS[mm - 1]; // mm/day → mm/month
      }
    }
    if (humidity) {
      for (const [key, val] of Object.entries(humidity)) {
        const mm = parseInt(key.substring(4));
        if (mm >= 1 && mm <= 12) { totalHumidity += val as number; humidityCount++; }
      }
    }

    if (monthlyMaxTemp.length !== 12 || monthlyMinTemp.length !== 12) {
      console.log('NASA POWER: incomplete monthly data');
      return null;
    }

    console.log(`NASA POWER climate: Tmax=${Math.max(...monthlyMaxTemp).toFixed(1)}°C Tmin=${Math.min(...monthlyMinTemp).toFixed(1)}°C precip=${totalPrecipMm.toFixed(0)}mm/yr humidity=${humidityCount > 0 ? (totalHumidity/humidityCount).toFixed(0) : '?'}%`);

    return {
      monthlyMaxTemp,
      monthlyMinTemp,
      annualPrecipMm: totalPrecipMm,
      avgHumidityPct: humidityCount > 0 ? Math.round(totalHumidity / humidityCount) : 50,
    };
  } catch (error) {
    console.error('NASA POWER climate fetch error:', error);
    return null;
  }
}
...
async function getOpenMeteoWindData(lat: number, lon: number, zone?: string, peakPower?: number, loss?: number): Promise<OpenMeteoWindResult | null> {
  try {
    ...
    const url = `https://archive-api.open-meteo.com/v1/archive?latitude=${lat}&longitude=${lon}&start_date=${startDate}&end_date=${endDate}&daily=wind_speed_10m_max,wind_speed_10m_mean,temperature_2m_max,temperature_2m_min,precipitation_sum,shortwave_radiation_sum,relative_humidity_2m_mean&wind_speed_unit=ms&timezone=auto`;
    ...
    const dailyHumidity = data.daily?.relative_humidity_2m_mean as number[] | undefined;
    ...
    for (let i = 0; i < dates.length; i++) {
      ...
      if (dailyHumidity?.[i] != null) { totalHumidity += dailyHumidity[i]; humidityCount++; }
...
// Open-Meteo Archive: get max SUSTAINED wind over 5 years for frame selection
// Uses wind_speed_10m_max (daily max of hourly-mean wind) — NOT gusts.
// Rationale: panel folding takes 60 sec, gusts (3-5 sec) pass before fold completes.
// Only sustained wind (≥1 hr average) is actionable for fold protection.
// Returns max sustained wind in km/h
async function getMaxGust5yr(lat: number, lon: number): Promise<{ maxGustKmh: number; dataYears: number } | null> {
  try {
    ...
    const url = `https://archive-api.open-meteo.com/v1/archive?latitude=${lat}&longitude=${lon}&start_date=${startDate}&end_date=${endDate}&daily=wind_speed_10m_max&wind_speed_unit=kmh&timezone=auto`;
    ...
    const data = await response.json();
    const speeds = data.daily?.wind_speed_10m_max as number[] | undefined;

    if (!speeds || speeds.length === 0) {
      console.error('No sustained wind data in Open-Meteo response');
      return null;
    }

    // Filter out null values and find max
    const validSpeeds = speeds.filter((s): s is number => s !== null && s !== undefined);
    const maxSustainedKmh = Math.max(...validSpeeds);
    const dataYears = endYear - startYear + 1;

    console.log(`Open-Meteo: Max sustained wind over ${dataYears} years = ${maxSustainedKmh.toFixed(1)} km/h (${validSpeeds.length} data points)`);

    return { maxGustKmh: maxSustainedKmh, dataYears };
  } catch (error) {
...
// WIND_MODEL_DERATING removed — conservative min(sources) replaces derating
...
async function getStationWindData(lat: number, lon: number): Promise<StationWindResult | null> {
  try {
    ...
    console.log(`Station wind: avg=${yearlyAvg.toFixed(1)} m/s`);
...
// Returns both conservative (min) and median per month
async function getWindData(lat: number, lon: number, zone: string, openMeteoWind?: WindDataResult | null): Promise<MergedWindResult | null> {
  ...
  // Conservative: min(sources) per month — for main recommendation
  const monthlyConservative = Array.from({ length: 12 }, (_, i) => {
    const values = results.map(r => r.monthlyAvg[i]).filter(v => v > 0);
    if (values.length === 0) return 0;
    return Math.min(...values);
  });
  const yearlyConservative = monthlyConservative.reduce((a, b) => a + b, 0) / 12;

  // Median: balanced estimate — for conditional scenario
  const monthlyMedian = Array.from({ length: 12 }, (_, i) => {
  ...
  const yearlyMedian = monthlyMedian.reduce((a, b) => a + b, 0) / 12;
  ...
  console.log(`Wind sources: ${sources} → conservative(min)=${yearlyConservative.toFixed(1)} median=${yearlyMedian.toFixed(1)} m/s`);
  ...
  return {
    conservative: {
      ...shared,
      monthlyAvg: monthlyConservative,
      yearlyAvg: yearlyConservative,
      dataSource: `Min(${results.map(r => r.dataSource).join(', ')})`,
    },
    median: {
      ...shared,
      monthlyAvg: monthlyMedian,
      yearlyAvg: yearlyMedian,
      dataSource: `Median(${results.map(r => r.dataSource).join(', ')})`,
    },
    sourceCount: results.length,
    confidence: {
      sources: results.length,
      level: results.length >= 2 ? 'medium' as const : 'low' as const,
      stationVerified: false,
      note: 'Model data only — subject to site wind verification',
    },
  };
}
...
function calculateSystemRequirements(
  ...
  const usesPVF180 = hideMode || maxGust > 120;
...
function calculatePrice(
  ...
  hide90: boolean = false,    // Hide 90° — force PVF90 (6 panels, fold 90° camouflage)
  annualPrecipMm: number = 999,  // Annual precipitation for ALD coating decision
  maxMonthlyTemp: number = 25,   // Hottest month avg daily max (°C) — cooling decision
  minMonthlyTemp: number = 5,    // Coldest month avg daily min (°C) — heating decision
  avgHumidityPct: number = 50,   // Avg relative humidity (%) — AWG decision
): {
  base: number;
  battery: number;
  wind: number;
  ...
  // - PVFB: 6 panels (4.32 kWp) — feathering, <80 km/h sustained wind
  // - PVF90: 6 panels — fold 90°, 80-120 km/h sustained wind
  // - PVF180: 4 panels (2.88 kWp) — fold 180°, >120 km/h sustained or desert
  // Note: uses sustained (hourly-mean) wind, not gusts — fold time 60s > gust duration 3-5s
  const usesPVF180 = hideMode || maxGust > 120;
  const usesPVF90 = hide90 || (!usesPVF180 && !usePolarFrame && maxGust >= 80);
  ...
  // LiFePO4 cost including thermal equipment — by actual climate, not zone
  let lfpThermalCost = 0;
  if (minMonthlyTemp < -15) {
  ...
  if (annualPrecipMm < 250 && originPref === 'World') {
    lfpThermalCost += panelQty * stations * COMPONENT_PRICES.ald_coating_per_panel;
  }
  ...
  const useLTO = ltoCost < lfpTotalCost && (maxMonthlyTemp >= 38 || minMonthlyTemp < -15);
  ...
  // Thermal components — by actual climate measures, NOT by zone label
  // SAHARA Thermal Kit (Compressor + PCM RT28 + AWG + Roll Bond): maxMonthlyTemp >= 38°C
  // ALD Al₂O₃ Coating: annualPrecipMm < 250 (sand/dust abrasion)
  // ARCTIC Thermal Kit (PTC + PCM RT10 + Iridium): minMonthlyTemp < -15°C
  const needsCooling = maxMonthlyTemp >= 38;
  const needsHeating = minMonthlyTemp < -15;
  // needsCoating already defined above
  const needsSeparateCoating = needsCoating && originPref === 'World';
  ...
  if (needsHeating) {
    if (useLTO) {
      addItem('Iridium Satellite Modem', 1, COMPONENT_PRICES.conn_iridium);
  ...
  // Cooling — SAHARA Thermal Kit (single thermodynamic circuit)
  if (needsCooling && !useLTO) {
  ...
  // ALD coating (arid / sand abrasion)
...
async function runNewSizingEngine(
  ...
  hideMode: boolean = false,
  hide90: boolean = false,
  monthlyTargetE_d?: number[],  // merged median E_d per month (kWh/day for 6 panels) — scale hourly to match
  ...
  try {
    ...
    // 1b. Scale hourly solar to match merged median from 3 sources
    if (monthlyTargetE_d && monthlyTargetE_d.length === 12) {
      // Compute actual monthly E_d from hourly pv_w (for 6 panels = peakPower kWp)
      const DAYS = [31,28,31,30,31,30,31,31,30,31,30,31];
      const monthlySum = new Array(12).fill(0);
      const monthlyHours = new Array(12).fill(0);
      for (const p of climate.data) {
        monthlySum[p.month] += p.pv_w / 1000;  // W → kWh (1 hour)
        monthlyHours[p.month]++;
      }
      for (let m = 0; m < 12; m++) {
        const actualE_d = monthlyHours[m] > 0 ? monthlySum[m] / DAYS[m] : 0;
        const targetE_d = monthlyTargetE_d[m];
        if (actualE_d > 0 && targetE_d > 0) {
          const k = targetE_d / actualE_d;
          // Scale pv_w for all hours in this month
          for (const p of climate.data) {
            if (p.month === m) p.pv_w *= k;
          }
        }
      }
      console.log('SizingEngine v4: hourly solar scaled to merged median');
    }
...
export async function POST(req: NextRequest) {
  try {
    ...
    const hideValue = body.hide;  // 'none' | '90' | '180' | boolean (legacy)
    const hideMode = hideValue === '180' || hideValue === true;  // PVF180 (4 panels, camouflage)
    const hide90 = hideValue === '90';  // PVF90 (6 panels, fold 90° camouflage)
    ...
    // Phase 1: Open-Meteo ERA5 (wind + climate + solar), gust, and NASA climate fallback — all parallel
    const [openMeteoResult, gust5yrData, nasaClimate] = await Promise.all([
      getOpenMeteoWindData(coords.lat, coords.lon, undefined, actualPeakPower, loss),
      getMaxGust5yr(coords.lat, coords.lon),
      getNASAClimateData(coords.lat, coords.lon),
    ]);

    // Phase 2: Determine zone from real climate data (Open-Meteo primary, NASA fallback)
    const zoneClimateSource = openMeteoResult?.climate || nasaClimate;
    if (zoneClimateSource) {
      const src = openMeteoResult?.climate ? 'Open-Meteo ERA5' : 'NASA POWER';
      const zr = detectZoneFromOpenMeteo(
        zoneClimateSource.monthlyMaxTemp,
        zoneClimateSource.monthlyMinTemp,
        zoneClimateSource.annualPrecipMm,
        coords.lat,
        zone !== 'continental' ? zone : undefined,
      );
      detectedZone = zr.zone;
      console.log(`Zone from ${src}: ${zr.zone} (Tmax=${zr.maxMonthlyTempC}°C, Tmin=${zr.minMonthlyTempC}°C, precip=${zoneClimateSource.annualPrecipMm.toFixed(0)}mm)`);
    } else {
      // Both unavailable — latitude only
      const zr = detectZoneFromLatitude(coords.lat);
      detectedZone = zr.zone;
      console.log(`Zone fallback (latitude): ${zr.zone}`);
    }

    // Climate measures — Open-Meteo ERA5 primary, NASA POWER fallback
    const climateSource = openMeteoResult?.climate || nasaClimate;
    const hasClimateData = !!climateSource;
    const annualPrecipMm = climateSource?.annualPrecipMm ?? 999;
    const minMonthlyTemp = climateSource?.monthlyMinTemp
      ? Math.min(...climateSource.monthlyMinTemp) : 5;
    const maxMonthlyTemp = climateSource?.monthlyMaxTemp
      ? Math.max(...climateSource.monthlyMaxTemp) : 25;
    const avgHumidityPct = climateSource?.avgHumidityPct ?? 50;
    if (!hasClimateData) {
      console.log('Warning: Both Open-Meteo and NASA climate data unavailable — no thermal extras');
    } else if (!openMeteoResult?.climate && nasaClimate) {
      console.log(`Climate fallback: using NASA POWER (Tmax=${maxMonthlyTemp.toFixed(1)}°C precip=${annualPrecipMm.toFixed(0)}mm humidity=${avgHumidityPct}%)`);
    }
    ...
    // Fallback: if PVGIS unavailable, build synthetic pvgisData from NASA/Open-Meteo solar
    let pvgisDataFinal = pvgisData;
    if (!pvgisDataFinal) {
      const fallbackSolar = nasaSolar || openMeteoResult?.solar || null;
      if (!fallbackSolar) {
        return NextResponse.json({
          status: 'error',
          message: 'Could not fetch solar data from any source. Try a nearby location.'
        }, { status: 400 });
      }
      console.log(`PVGIS unavailable — building fallback from ${fallbackSolar.dataSource}`);
      const DAYS = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];
      const fallbackMonthly: MonthlyAggregate[] = fallbackSolar.monthlyIrradiance.map((H_d, i) => ({
        month: i + 1,
        E_d: fallbackSolar.monthlyProduction[i],
        E_m: fallbackSolar.monthlyProduction[i] * DAYS[i],
        H_d,
        H_m: H_d * DAYS[i],
        avgTemp: 15,
        avgWind: 0,
      ...
      pvgisDataFinal = {
        ...
        yearly: {
          E_y: fallbackSolar.yearlyProduction,
          E_d: fallbackSolar.yearlyProduction / 365,
          H_y: fallbackSolar.monthlyIrradiance.reduce((sum, v, i) => sum + v * DAYS[i], 0),
        },
        location: { lat: coords.lat, lon: coords.lon, elevation: 0 },
        database: fallbackSolar.dataSource,
      };
    }

    // Merge multi-source solar irradiance and overwrite pvgisDataFinal monthly
    ...
    if (mergedSolar) {
      const DAYS = [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];
      for (let i = 0; i < 12 && i < pvgisDataFinal.monthly.length; i++) {
        pvgisDataFinal.monthly[i].H_d = mergedSolar.monthlyIrradiance[i];
        pvgisDataFinal.monthly[i].E_d = mergedSolar.monthlyProduction[i];
        pvgisDataFinal.monthly[i].H_m = mergedSolar.monthlyIrradiance[i] * DAYS[i];
        pvgisDataFinal.monthly[i].E_m = mergedSolar.monthlyProduction[i] * DAYS[i];
      }
      // Update yearly totals
      pvgisDataFinal.yearly.E_y = mergedSolar.yearlyProduction;
      pvgisDataFinal.yearly.E_d = mergedSolar.yearlyProduction / 365;
      pvgisDataFinal.yearly.H_y = mergedSolar.monthlyIrradiance.reduce((sum, v, i) => sum + v * DAYS[i], 0);
      console.log(`Solar merged: ${mergedSolar.dataSource}, yearly=${mergedSolar.yearlyProduction.toFixed(0)} kWh`);
    }

    // Merge wind data into monthly aggregates — use CONSERVATIVE (min sources) for main recommendation
    if (windData) {
      const windForSizing = windData.conservative;
      for (let i = 0; i < pvgisDataFinal.monthly.length; i++) {
        pvgisDataFinal.monthly[i].avgWind = windForSizing.monthlyAvg[i] || 0;
        pvgisDataFinal.monthly[i].windProduction = calculateDailyWindEnergy(windForSizing.monthlyAvg[i] || 0);
      }
    }

    // Wind metrics — conservative for main recommendation
    const avgWindSpeed = windData?.conservative.yearlyAvg || 0;
    const maxHourlyWind = windData?.conservative.maxHourlyWind || 0;
    const estimatedMaxGust = windData?.conservative.estimatedMaxGust || 0;
    const gustFactor = windData?.conservative.gustFactor || 1.3;

    // 5-year max SUSTAINED wind from Open-Meteo Archive (in km/h)
    // Uses daily max of hourly-mean wind, not gusts (fold time 60s > gust duration 3-5s)
    const maxGust5yrKmh = gust5yrData?.maxGustKmh || 0;
    const gust5yrYears = gust5yrData?.dataYears || 0;
    console.log(`Frame selection: zone=${detectedZone}, max_sustained_5yr=${maxGust5yrKmh.toFixed(1)} km/h`);
    ...
    // Do NOT scale pvgisDataFinal here to avoid double-scaling.

    // === TRY NEW SIZING ENGINE (v4) ===
    // Pass legacy 3-source wind data (more accurate than single-source PVGIS seriescalc)
    // Extract merged monthly E_d for v4 engine scaling
    const mergedMonthlyE_d = pvgisDataFinal.monthly.map(m => m.E_d);
    ...
    const systemReqs = calculateSystemRequirements(dailyKwh, pvgisDataFinal.monthly, detectedZone, avgWindSpeed, loadClass, dailySeries, coords.lat, maxGust5yrKmh, mode, grid, hideMode, hide90);
    ...
    if (hideMode) {
      ...
      frameType = 'PVF90';
      foldLevel = 'Hide — Fold 90° (camouflage)';
      frameReason = 'Hide mode — 6 panels fold 90° for low-profile operation';
      ...
      frameReason = `5-year max sustained wind ${maxGust5yrKmh.toFixed(0)} km/h > 120 km/h`;
    } else if (maxGust5yrKmh >= 80) {
      frameType = 'PVF90';
      foldLevel = 'Level 2 — Fold 90°';
      frameReason = `5-year max sustained wind ${maxGust5yrKmh.toFixed(0)} km/h (80-120 km/h range)`;
    } else {
      frameType = 'PVFB';
      foldLevel = 'Level 1 — Feathering';
      frameReason = `5-year max sustained wind ${maxGust5yrKmh.toFixed(0)} km/h < 80 km/h`;
    }
    ...
    const windSourceCount = windData?.sourceCount || 0;
    ...
    return NextResponse.json({
      ...
      climate: {
        ...
        max_sustained_5yr_kmh: maxGust5yrKmh,
        max_gust_5yr_kmh: maxGust5yrKmh, // backward compat
        gust_5yr_years: gust5yrYears,
        gust_source: gust5yrData ? 'Open-Meteo Archive (sustained)' : 'estimated',
        turbine_height_m: windData?.conservative.turbineHeight || 7,
        wind_data_source: windData?.conservative.dataSource || 'PVGIS',
        wind_sources: {
          conservative: windData ? { yearly_ms: +windData.conservative.yearlyAvg.toFixed(1), source: windData.conservative.dataSource } : null,
          median: windData ? { yearly_ms: +windData.median.yearlyAvg.toFixed(1), source: windData.median.dataSource } : null,
          used_for_sizing: +(avgWindSpeed).toFixed(1),
          sizing_method: 'conservative (min of sources)',
          recommendation: avgWindSpeed >= 4.0 ? 'VAWT recommended — conservative wind ≥ 4 m/s' :
                          avgWindSpeed >= 3.0 ? 'VAWT marginal — on-site verification needed' :
                          'VAWT not recommended — insufficient wind resource',
        },
        windConfidence: windData?.confidence || { sources: 0, level: 'low', stationVerified: false, note: 'No wind data' },
        worst_month: worstMonthName,
        best_month: bestMonthName,
        pvgis_source: pvgisDataFinal.database,
        solar_sources: mergedSolar?.dataSource || pvgisDataFinal.database,
        wind_sources_desc: windData?.conservative.dataSource || 'PVGIS',
        climate_source: openMeteoResult?.climate ? 'Open-Meteo ERA5' : (nasaClimate ? 'NASA POWER' : 'N/A'),
        min_temp_c: +minMonthlyTemp.toFixed(1),
        max_temp_c: +maxMonthlyTemp.toFixed(1),
      },
      location: {
        address: coords.display || address || `${coords.lat.toFixed(4)}, ${coords.lon.toFixed(4)}`,
        lat: pvgisDataFinal.location.lat,
        lon: pvgisDataFinal.location.lon,
        elevation: pvgisDataFinal.location.elevation,
      ...
      solarData: {
        ...
        tracking: '2-axis',
        database: pvgisDataFinal.database,
        yearlyPerStation: pvgisDataFinal.yearly.E_y,
        dailyAverage: pvgisDataFinal.yearly.E_d
      },
      analysis: {
      ...
      solution: {
        ...
        thermal: [
          ...(minMonthlyTemp < -15 ? ['PTC Heating Film', 'PCM RT10HC'] : []),
          ...(maxMonthlyTemp >= 38 ? ['DC Compressor', 'PCM RT28HC', 'AWG Module', 'Roll Bond'] : []),
        ],
        connectivity: minMonthlyTemp < -15 ? ['4G LTE', 'Iridium Satellite'] : ['4G LTE'],
      ...
      energy: (() => {
        ...
        const monthlyRows = pvgisDataFinal.monthly.map((m, i) => {
      ...
      insights: {
        ...
        coordinatesDMS: {
          lat: `${Math.abs(pvgisDataFinal.location.lat).toFixed(4)}°${pvgisDataFinal.location.lat >= 0 ? 'N' : 'S'}`,
          lon: `${Math.abs(pvgisDataFinal.location.lon).toFixed(4)}°${pvgisDataFinal.location.lon >= 0 ? 'E' : 'W'}`
        },
        // Climate description
        ...
        solarResource: pvgisDataFinal.yearly.E_d > 25 ? 'Excellent' :
                       pvgisDataFinal.yearly.E_d > 20 ? 'Very Good' :
                       pvgisDataFinal.yearly.E_d > 15 ? 'Good' :
                       pvgisDataFinal.yearly.E_d > 10 ? 'Moderate' : 'Low',
        // Irradiance (kWh/m²/day average)
        irradiance: (pvgisDataFinal.yearly.H_y / 365).toFixed(2),
      ...
      monthlyBreakdown: pvgisDataFinal.monthly.map(m => {
