...
interface PVGISResult {
  ...
  climate: {
    ...
    best_month: string;
    pvgis_source: string;
    solar_sources?: string;
    wind_sources_desc?: string;
...
export default function ProductsShowcase({ onComplete }: ProductsShowcaseProps) {
  ...
  // Order modal state
  const [orderModal, setOrderModal] = useState<{ open: boolean; scheme: 'credit' | 'eaas' }>({ open: false, scheme: 'credit' });
    ...
    >
                    ...
                    <>
                      ...
                      <div className="bg-zinc-50 rounded-lg p-3 max-w-[280px]">
                        ...
                        <div className="text-[9px] text-zinc-400 space-y-0.5">
                          <div>☀️ {pvgisResult.climate.solar_sources || pvgisResult.climate.pvgis_source}</div>
                          <div>💨 {pvgisResult.climate.wind_sources_desc || 'PVGIS TMY'}</div>
                          <div>🌪️ {pvgisResult.climate.gust_source || 'Open-Meteo Archive'}</div>
                          <div>🌡️ {pvgisResult.climate.climate_source || 'Open-Meteo ERA5'}</div>
                        </div>
                      </div>
                    </>
                  )}
                    ...
                    <div className="bg-white rounded-xl p-4 shadow-sm">
                      ...
                      {pvgisResult.pricing.financing && (
                        <div className="border-t border-zinc-200 mt-3 pt-3 space-y-2">
                          <button
                            onClick={() => setOrderModal({ open: true, scheme: 'credit' })}
                            className="w-full text-left hover:bg-zinc-50 rounded-lg p-2 -mx-2 transition-colors group"
                          >
                            <div className="flex justify-between text-xs">
                              <span className="text-zinc-500 group-hover:text-zinc-700">Credit purchase</span>
                              <span className="text-zinc-700 font-medium">€{pvgisResult.pricing.financing.credit.monthly_payment_eur}/mo</span>
                            </div>
                            <div className="flex justify-between text-xs mt-0.5">
                              <span className="text-zinc-400">30% down + 10yr/10%</span>
                              <span className="text-zinc-400">€{pvgisResult.pricing.financing.credit.cost_per_kwh_eur}/kWh</span>
                            </div>
                          </button>
                          <button
                            onClick={() => setOrderModal({ open: true, scheme: 'eaas' })}
                            className="w-full text-left hover:bg-zinc-50 rounded-lg p-2 -mx-2 transition-colors group"
                          >
                            <div className="flex justify-between text-xs">
                              <span className="text-zinc-500 group-hover:text-zinc-700">EaaS subscription</span>
                              <span className="text-zinc-700 font-medium">€{pvgisResult.pricing.financing.eaas.monthly_payment_eur}/mo</span>
                            </div>
                            <div className="flex justify-between text-xs mt-0.5">
                              <span className="text-zinc-400">No upfront cost</span>
                              <span className="text-zinc-400">€{pvgisResult.pricing.financing.eaas.cost_per_kwh_eur}/kWh</span>
                            </div>
                          </button>
                        </div>
                      )}
            ...
            >
              ...
              <div className={`grid grid-cols-1 ${pvgisResult ? 'lg:grid-cols-[280px_340px_380px] max-w-[1040px]' : 'lg:grid-cols-[280px_400px] max-w-[720px]'} gap-4 sm:gap-6 mx-auto`}>
                    ...
                    <>
                      ...
                      <div className="bg-white rounded-xl p-4 max-w-[280px] shadow-sm">
                        ...
                        <div className="space-y-2 text-xs">
                          ...
                          <div className="flex justify-between">
                            <span className="text-zinc-500">Min Temp</span>
                            <span className="text-zinc-700 font-medium">{pvgisResult.climate.min_temp_c}°C</span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-zinc-500">Max Temp</span>
                            <span className="text-zinc-700 font-medium">{pvgisResult.climate.max_temp_c}°C</span>
                          </div>
                          <div className="flex justify-between pt-2 border-t border-zinc-100">
                ...
                >
                  ...
                  {/* Output options - vertical table layout */}
                  <div className="flex gap-4">
                    {/* DC Column */}
                    <div className="flex flex-col items-center gap-0.5">
                      <span className="text-[10px] text-zinc-400 uppercase tracking-wide font-medium mb-1">DC</span>
                      {['48', '24', '12'].map((v) => {
                        const isSelected = config.voltageTypes.includes('DC') && config.voltageLevelDC === v;
                        return (
                          <button
                            key={v}
                            onClick={() => setConfig(prev => {
                              if (isSelected) {
                                return { ...prev, voltageTypes: prev.voltageTypes.filter(t => t !== 'DC') };
                              }
                              return {
                                ...prev,
                                voltageTypes: [...prev.voltageTypes.filter(t => t !== 'DC'), 'DC'],
                                voltageLevelDC: v,
                              };
                            })}
                            className={`text-xs font-medium transition-all duration-200 px-3 py-1 rounded w-full ${
                              isSelected
                    ...
                    {/* AC Column */}
                    <div className="flex flex-col items-center gap-0.5">
                      <span className="text-[10px] text-zinc-400 uppercase tracking-wide font-medium mb-1">AC</span>
                      {[
                        { v: '110', p: '1' as const, label: '110V 1P' },
                        { v: '230', p: '1' as const, label: '230V 1P' },
                        { v: '400', p: '3' as const, label: '400V 3P' },
                      ].map((opt) => {
                        const isSelected = config.voltageTypes.includes('AC') && config.voltageLevelsAC.includes(opt.v + 'V') && config.acPhase === opt.p;
                        return (
                          <button
                            key={opt.v}
                            onClick={() => setConfig(prev => {
                              if (isSelected) {
                                return { ...prev, voltageTypes: prev.voltageTypes.filter(t => t !== 'AC'), voltageLevelsAC: [] };
                              }
                              return {
                                ...prev,
                                voltageTypes: [...prev.voltageTypes.filter(t => t !== 'AC'), 'AC'],
                                voltageLevelsAC: [opt.v + 'V'],
                                acPhase: opt.p,
                              };
                            })}
                            className={`text-xs font-medium transition-all duration-200 px-3 py-1 rounded w-full whitespace-nowrap ${
                              isSelected
                                ? 'text-zinc-900 bg-zinc-100'
                                : 'text-zinc-400 hover:text-zinc-600'
                            }`}
                    ...
                    {/* Origin Column - Globe */}
                    <div className="flex flex-col items-center gap-0.5">
                      <span className="text-[10px] text-zinc-400 mb-1 cursor-default" title="Country of Origin">
                        <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                          <path strokeLinecap="round" strokeLinejoin="round" d="M12 21a9.004 9.004 0 008.716-6.747M12 21a9.004 9.004 0 01-8.716-6.747M12 21c2.485 0 4.5-4.03 4.5-9S14.485 3 12 3m0 18c-2.485 0-4.5-4.03-4.5-9S9.515 3 12 3m0 0a8.997 8.997 0 017.843 4.582M12 3a8.997 8.997 0 00-7.843 4.582m15.686 0A11.953 11.953 0 0112 10.5c-2.998 0-5.74-1.1-7.843-2.918m15.686 0A8.959 8.959 0 0121 12c0 .778-.099 1.533-.284 2.253m0 0A17.919 17.919 0 0112 16.5a17.92 17.92 0 01-8.716-2.247m0 0A9 9 0 013 12c0-1.605.42-3.113 1.157-4.418" />
                        </svg>
                      </span>
                      {(['World', 'EU'] as const).map((origin) => (
                        <button
                          key={origin}
                          onClick={() => setConfig(prev => ({ ...prev, components: origin }))}
                          className={`text-xs font-medium transition-all duration-200 px-3 py-1 rounded w-full text-center ${
                    ...
                    {/* Hide Column - Eye */}
                    <div className="flex flex-col items-center gap-0.5">
                      <span className="text-[10px] text-zinc-400 mb-1 cursor-default" title="HIDDEN">
                        <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                      ...
                      {/* Open eye - no hide */}
                      <button
                        onClick={() => setConfig(prev => ({ ...prev, hide: 'none' as const }))}
                      ...
                      {/* Eye with slash - 90° fold */}
                      <button
                        onClick={() => setConfig(prev => ({ ...prev, hide: '90' as const }))}
                        className={`transition-all duration-200 px-3 py-1 rounded ${
                          config.hide === '90'
                            ? 'text-zinc-900 bg-zinc-100'
                            : 'text-zinc-400 hover:text-zinc-600'
                      ...
                      {/* Closed eye - 180° fold */}
                      <button
                        onClick={() => setConfig(prev => ({ ...prev, hide: '180' as const }))}
                        className={`transition-all duration-200 px-3 py-1 rounded ${
                          config.hide === '180'
                            ? 'text-zinc-900 bg-zinc-100'
                            : 'text-zinc-400 hover:text-zinc-600'
                    ...
                    <div className="p-4 border-t border-zinc-200 bg-zinc-50">
                      ...
                      {pvgisResult.pricing.financing && (
                        <div className="border-t border-zinc-200 mt-3 pt-3 grid grid-cols-2 gap-3">
                          <button
                            onClick={() => setOrderModal({ open: true, scheme: 'credit' })}
                            className="text-left hover:bg-white rounded-lg p-2 -m-1 transition-colors group"
                          >
                            <p className="text-zinc-400 text-xs mb-1 group-hover:text-zinc-600">Credit purchase</p>
                          ...
                          <button
                            ...
                            className="text-left hover:bg-white rounded-lg p-2 -m-1 transition-colors group"
                          >
                            <p className="text-zinc-400 text-xs mb-1 group-hover:text-zinc-600">EaaS subscription</p>
      ...
      {pvgisResult && (
        <OrderModal
          ...
          scheme={orderModal.scheme}
          location={pvgisResult.location.address}
          zone={pvgisResult.zone}
          mode={view === 'civil' ? 'Civil' : 'Industrial'}
          model={pvgisResult.solution.model}
          stations={pvgisResult.solution.stations}
          totalPrice={pvgisResult.pricing.retail_eur}
          monthlyPayment={
            orderModal.scheme === 'credit'
              ? pvgisResult.pricing.financing?.credit.monthly_payment_eur || 0
              : pvgisResult.pricing.financing?.eaas.monthly_payment_eur || 0
          }
        />
      )}
