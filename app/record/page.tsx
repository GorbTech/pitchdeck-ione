...
// Same messaging components from TechDemo but without navigation
...
// Scene timings from URL params (in seconds)
// ?scene=0 (sec0), ?scene=1 (messaging), ?scene=2 (mission)
...
export default function RecordPage() {
  ...
  useEffect(() => {
    ...
    if (sceneParam !== null) {
      setScene(parseInt(sceneParam, 10));
    }
  }, []);

  // Messaging animation for scene 1
  useEffect(() => {
    if (scene !== 1 || stepIndex >= SCENARIO.length) return;
    ...
    return (
      <div className="fixed inset-0 bg-black flex items-center justify-center text-white">
        <div className="text-center">
          <h1 className="text-2xl mb-4">Recording Mode</h1>
          <p>Add ?scene=0, ?scene=1, or ?scene=2 to URL</p>
        </div>
      </div>
    );
  }

  // Scene 0: Fleet Intelligence (iframe)
  if (scene === 0) {
    return (
  ...
  // Scene 1: Predictive Service (messaging)
  if (scene === 1) {
    return (
