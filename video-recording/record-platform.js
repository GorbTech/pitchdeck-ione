...
const DURATION = 165000; // 165 seconds (fixed)

async function record() {
    console.log('🎬 Recording platform-auto.html...');
    ...
    console.log(`   Captured ${frames} frames (${Math.round((Date.now() - start) / 1000)}s)`);
    ffmpeg.stdin.end();
    await new Promise(r => ffmpeg.on('close', r));
    await browser.close();

    console.log('   Merging video + audio...');
    execSync(`ffmpeg -y -i /home/ubuntu/gtfiles/storage/admin/video/platform-video.mp4 -i /tmp/platform-audio/full-audio.m4a -c:v copy -c:a aac -map 0:v -map 1:a -shortest "${OUTPUT}" 2>/dev/null`);
